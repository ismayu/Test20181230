...
typedef int (*TIMERCALLBACK)(timer* );
typedef int (*CLASSTIMERCALLBACK)(void* frame, timer*);
 ...
 */
template<typename T>
 int timers::AddTimer(double internalvalue, T callback, void *data, int rep){
    timer* task = new timer(internalvalue, (callback), data, rep);
...
template<typename T>
int timers::AddTimer(double internalvalue, T callback, void* frame, void* data, int rep){
    timer* task = new timer(internalvalue, (callback), frame, data, rep);
    ...
    for(; mactive; ){
        ...
        for(int i = 0; i < nfds; ++i){
            ...
            if(itmp != timersMap.end()){
                ...
                if (itmp->second->GetFrame() == NULL) {
                    (itmp->second->GetUserCallback())();
                    printf("%s\n", (char *) itmp->second->GetUserData());
                    // ((TIMERCALLBACK) itmp->second->GetUserCallback())(itmp->second);

                    
                } else {
                    // (itmp->second->GetUserCallback())(itmp->second->GetFrame());
                    // (itmp->second->GetUserCallback())();
                    // ((CLASSTIMERCALLBACK) itmp->second->GetUserCallback())(itmp->second->GetFrame(),itmp->second);
                }
